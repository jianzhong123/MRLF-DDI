...
from gridsearch_deng_S0_data_helper import construct_graph, get_kg
...
def train(model, train_data_loader, index, test_data_loader,loss_fn,  optimizer, n_epochs, device, scheduler=None):
    ...
    for i in range(1, n_epochs+1):
        ...
        with torch.no_grad():
            ...
            if best_f1 < train_f1:
                best_f1 = train_f1
                torch.save(model.state_dict(), 'S0_MGFF_DDIMDL_cross' + str(index) + '.pkl')
                print('model saved')
                predlabls += 1
                truelabels += 1
...
if __name__ == '__main__':
    ...
    def run_model():#params
        ...
        for index in range(1,6):
            ###### Dataset
            df_ddi_train = pd.read_csv('../data/Dengcross5datasets/Deng_S0/S0_DDIMDL_idx_train_cross_'+str(index)+'.csv')
            ...
            df_ddi_test = pd.read_csv('../data/Dengcross5datasets/Deng_S0/S0_DDIMDL_idx_test_cross_' + str(index) + '.csv')
