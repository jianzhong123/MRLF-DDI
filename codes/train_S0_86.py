...
from model.modelmlp_S0_86 import model_S0
...
from S0data_helper import construct_graph, get_kg
...
def do_compute_metrics(probas_pred, target):
    ...
    y_pred_pro = np.array(y_pred_pro).reshape((-1))
    ...
    return acc, f1_score1, recall1,precision1,kappa,aaa,bbb,y_pred_pro
...
def train(model, train_data_loader, val_data_loader, test_data_loader,loss_fn,  optimizer, n_epochs, device, scheduler=None):
    ...
    for i in range(1, n_epochs+1):
        ...
        for batch in train_data_loader:
            ...
            outs,autoloss,_,_=model(drug1s, drug2s,graph)
        ...
        with torch.no_grad():
            ...
            train_acc, train_f1, train_recall,train_precision,kappa,_,_,_ = do_compute_metrics(train_probas_pred, train_ground_truth)
            ...
            val_acc, val_f1, val_recall, val_precision,val_kappa,_,_,_= do_compute_metrics(val_probas_pred, val_ground_truth)
            # if scheduler:
            scheduler.step()

            # if val_precision > best_acc:
            #     best_acc = val_precision
            #     #np.save('allfeatures.npy',all_emb.cpu().detach().numpy())
            #     torch.save(model.state_dict(), 'label86_S0_best.pt')
            #     print('model saved')
            ...
            for batch in test_data_loader:
                ...
                drug1s, drug2s, labels = batch
                outs,_,modelouts,initouts= model(drug1s, drug2s, graph)
                #outs= torch.sigmoid(outs)
                probas_pred = outs.detach().cpu().numpy()
                labels = labels.detach().cpu().numpy()
                test_probas_pred.append(np.array(probas_pred))
                test_ground_truth.append(labels)
                all_drug1s.append(drug1s.detach().cpu().numpy())
                all_drug2s.append(drug2s.detach().cpu().numpy())
                all_outs.append(outs.detach().cpu().numpy())
                all_modelouts.append(modelouts.detach().cpu().numpy())
                all_initouts.append(initouts.detach().cpu().numpy())
            test_probas_pred = np.concatenate(test_probas_pred)
            test_ground_truth = np.concatenate(test_ground_truth)

            test_acc, test_f1, test_recall, test_precision, test_kappa, predlabls,truelabels,predpros = do_compute_metrics(test_probas_pred,test_ground_truth)
...
if __name__ == '__main__':
    ...
    def run_model():#params
        ...
        ###### Dataset
        df_ddi_train = pd.read_csv('../data/86datasets/S0_drug1559_train86.csv',header=0)
        ...
        df_ddi_val = pd.read_csv('../data/86datasets/S0_drug1559_valid86.csv',header=0)
        ...
        valdata = Data.TensorDataset(val_drug1, val_drug2, val_label)

        df_ddi_test = pd.read_csv('../data/86datasets/S0_drug1559_test86.csv', header=0)
