...
class model_S0(torch.nn.Module):
    def __init__(self,device):#in_dim,hidden_dim,out_dim,
        ...
        self.mlp = nn.ModuleList([nn.Linear(1024, 1024),
                                  nn.ELU(),
                                  nn.Linear(1024, 86)
                                  ])
        ...
        alignn_path = '../data/drugfeatures/S0_drugs1559_alignn86.npy'
        ...
        input_features = torch.cat((pub, morgen, alignn), dim=1)
    ...
    def forward(self, drug1s,drug2s, graph):
        ...
        init_emb=torch.cat((drug1_emb, drug2_emb), 1)
        drug1_global_emb, drug2_global_emb = self.gnn(drug1s, drug2s, graph, en_emb1)
        ...
        #all=fusion2
        out = self.MLP(all, 3)
        return out,autoloss,all,init_emb
