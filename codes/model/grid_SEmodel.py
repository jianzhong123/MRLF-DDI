...
class SE_GNN(nn.Module):
    def __init__(self,h_dim=128,l=1):
        ...
        self.kg_n_layer = l
        # relation SE layer
        self.edge_layers = nn.ModuleList([EdgeLayer(128) for _ in range(self.kg_n_layer)])
        # entity SE layer
        self.node_layers = nn.ModuleList([NodeLayer(128) for _ in range(self.kg_n_layer)])
        # triple SE layer
        self.comp_layers = nn.ModuleList([CompLayer(128) for _ in range(self.kg_n_layer)])
        rel_w =  Parameter(torch.zeros(128*l, 128))
        self.rel_w = xavier_normal_(rel_w)
        # relation embedding for aggregation
        param = Parameter(torch.zeros(self.n_rel * 2, 128))
